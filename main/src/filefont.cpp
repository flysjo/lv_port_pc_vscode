...
extern "C" const void * my_get_glyph_bitmap_cb(lv_font_glyph_dsc_t *, lv_draw_buf_t *);
static lv_font_glyph_format_t formatFromBpp(int bpp)
{
    switch(bpp) {
        case 1: return LV_FONT_GLYPH_FORMAT_A1;
        case 2: return LV_FONT_GLYPH_FORMAT_A2;
        case 3: return LV_FONT_GLYPH_FORMAT_A3;
        case 4: return LV_FONT_GLYPH_FORMAT_A4;
        case 8: return LV_FONT_GLYPH_FORMAT_A8;
        default: return LV_FONT_GLYPH_FORMAT_NONE;
    }
}
static lv_font_fmt_txt_cmap_type_t typeFromFormat(int format)
{
    switch(format) {
        case LV_FONT_FMT_TXT_CMAP_FORMAT0_FULL: return LV_FONT_FMT_TXT_CMAP_FORMAT0_FULL;
        case LV_FONT_FMT_TXT_CMAP_SPARSE_TINY: return LV_FONT_FMT_TXT_CMAP_SPARSE_TINY;
        case LV_FONT_FMT_TXT_CMAP_SPARSE_FULL: return LV_FONT_FMT_TXT_CMAP_SPARSE_FULL;
        default:
        case LV_FONT_FMT_TXT_CMAP_FORMAT0_TINY: return LV_FONT_FMT_TXT_CMAP_FORMAT0_TINY;
    }
}
#endif
...
namespace els::cpro2::common::util::font
{
    ...
    FileFont::FileFont() : file_(nullptr), size_(0)
    {
    ...
    FileFont::~FileFont()
    {
        ...
        if (font_.dsc)
        {
            ...
            if (font_dsc->cmaps)
            {
                ...
                {
                    if (font_dsc->cmaps[i].unicode_list) delete [] font_dsc->cmaps[i].unicode_list;
    ...
    const void * FileFont::get_glyph_bitmap(lv_font_glyph_dsc_t * g_dsc, lv_draw_buf_t * draw_buf)
    {
    ...
    bool FileFont::load_cmaps_tables(lv_font_fmt_txt_dsc_t * font_dsc, uint32_t cmaps_start, cmap_table_bin_t * cmap_table)
    {
        ...
        for (unsigned int i = 0; i < font_dsc->cmap_num; ++i) {
            ...
            // cmap->type = typeFromFormat(cmap_table[i].format_type);
    ...
    {
        return 0 == fseek(file_, pos, SEEK_SET);
    }
    ...
    {
        return fread(buffer, size, 1, file_) > 0;
    }
...
using namespace els::cpro2::common::util::font;
...
#elif LV_VERSION_CHECK(9, 0, 0)
extern "C" const void * my_get_glyph_bitmap_cb(lv_font_glyph_dsc_t * g_dsc, lv_draw_buf_t * draw_buf)
{
    FileFont *fontPtr = reinterpret_cast<FileFont*>(g_dsc->entry);
    return fontPtr->get_glyph_bitmap(g_dsc, draw_buf);
}
#endif
