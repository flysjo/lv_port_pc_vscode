...
namespace els::cpro2::common::platform::gui::images
{
   ...
   bool PngToBin(const char *src_filename, const char *filename)
   {
      ...
      if (inFile.Open(src_filename, FA_READ) == FR_OK)
      {
         ...
         lodepng_decode32(&imageBuffer, &width, &height, buffer.data(), fileSize);
   ...
   FileImage::FileImage()
   {
      img_.header.cf = LV_IMG_CF_UNKNOWN;
   ...
   bool FileImage::load(const char* filename)
   {
      file_monitor::MonitoredFile inFile;
      inFile.Open(filename, FA_READ);
      if (!inFile.IsOpen())
      {
         return false;
      }
      size_t readSize = 0;
      inFile.Read(&img_.header, sizeof(img_.header), readSize);
      img_.data_size = img_.header.w * img_.header.h * 3;
      if (img_.data_size == 0)
      {
         inFile.Close();
         return false;
      }
      img_.data = new uint8_t[img_.data_size];
      inFile.Read(const_cast<uint8_t*>(img_.data), img_.data_size, readSize);
      inFile.Close();
      return true;
   }
